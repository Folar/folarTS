...
let dbInfoHive = {
    ...
    user: "tdr1",
    password: "paco468paco",
    database: "tracker1db"
}

let token = '1525746258345';
let syms = [{
    s: 'rut',
    oh: 6,
    om: 27,
...
function writeTempFiles(map, sym, tm) {
    var fs = require('fs');
    let dir = __dirname + "/";
    //let fn = dir + tbl + ".csv";
    ...
    for (tbl in map) {
        ...
        var logStream = fs.createWriteStream(fn, {'flags': 'w'});
...
function make_dir() {
    ...
    if (isOpenToday() && (currentDate == null || currentDate != dt)) {
        currentDate = dt;
        currentPath = "/Users/lawrenceackner/Qsync/Folar/" + currentDate;
...
function populate_quotes() {
    setTimeout(populate_quotes, 150 * 1000);
    ...
    for (i in syms) {
        ...
        if (!active) {
            ...
            let tablesToCreate = [];
            tbl_cnt = 0;
            getFromAmeritrade(table_names, syms[i].s, 80, map, tablesToCreate, dt, tm);
        }
